...
const customSeriesOrder = [
  "2026 情人節バレンタイン＆ホワイトデー","2026 節分","2026 干支午年","2026 JUMPFES",
  "2025-2026 誕生祭","2025 聖誕節","2025 AGF·EARLYWINTER","2025 鬼滅百景·柱百景＆隊士百景","2025 京まふ","2025 築地銀だこ章魚燒","2025 情人節バレンタイン＆ホワイトデー","2025 干支巳年",
  "2024-2025 誕生祭","2024 聖誕節","2024 AGF·純喫茶","2024 柱展","2024 京まふ","2024 動畫伍周年記念祭＆後夜祭","2024 情人節バレンタイン＆ホワイトデー","2024 干支辰年",
  "2023-2024 誕生祭","2023 聖誕節","2023 AGF·干支卯年","2023 京まふ","2023 情人節バレンタイン＆ホワイトデー",
  "2022-2023 誕生祭","2022 聖誕節","2022 情人節バレンタイン＆ホワイトデー",
  "2021 聖誕節","2021 京まふ",
  "2020 聖誕節"
];
...
const customTypeOrder = [
  ...
  "壁掛","居家用品","首飾、髮飾","包、袋","衣物","推活小物","其他"
];

const customCharacterOrder = [
  "富岡義勇","竈門炭治郎","竈門禰豆子","我妻善逸","嘴平伊之助","不死川玄彌","栗花落香奈乎",
  "胡蝶忍","煉獄杏壽郎","宇髓天元","時透無一郎","甘露寺蜜璃","不死川實彌","伊黑小芭內",
  "悲鳴嶼行冥","產屋敷耀哉","鬼無辻無慘","黑死牟","童磨","猗窩座","半天狗","積怒","可樂",
  "空喜","哀絕","憎珀天","玉壺","妓夫太郎","墮姬","獪岳","鳴女","錆兔","真菰","鱗瀧左近次",
  "珠世","愈史郎","村田","狛治","戀雪","慶藏","胡蝶香奈惠","鋼鐵塚","小鐵","神崎葵","雛鶴",
  "牧緒","須磨","桑島慈悟郎","產屋敷輝利哉","肌肉鼠","魘夢","累","竈門炭十郎"
];
...
const detailConfig = {
  ...
  "柱展·百景":["2025 鬼滅百景·柱百景＆隊士百景","2024 柱展"],
  "聖誕節":["2025 聖誕節","2024 聖誕節","2023 聖誕節","2022 聖誕節","2021 聖誕節","2020 聖誕節"],
  "情人節":["2026 情人節バレンタイン＆ホワイトデー","2025 情人節バレンタイン＆ホワイトデー","2024 情人節バレンタイン＆ホワイトデー","2023 情人節バレンタイン＆ホワイトデー"]
};
...
function populateFilters(data){
  ...
  // 年份
  [...yearSet].sort((a,b)=>b-a).forEach(y=>yearFilter.insertAdjacentHTML("beforeend", `<option value="${y}">${y}</option>`));
}
...
function displayProducts(items, reset=true){
    ...
    const card = `
      <div class="card" id="product-${item.id}">
        ...
        <div class="card-buttons">
          <button class="favorite-btn ${favorites.has(item.id)?'favorited':''}" data-id="${item.id}">
  ...
  const newCards = Array.from(productList.querySelectorAll(".card")).slice(-items.length).map(c=>c.querySelector(".image-slider"));
...
function addSliderEvents(sliders){
  sliders.forEach(slider=>{
    ...
    let index = 0;
    const showImage = i=>{images.forEach(img=>img.classList.remove("active")); images[i].classList.add("active");};
...
document.addEventListener("change", e=>{ 
  if(e.target.classList.contains("owned-checkbox")) toggleOwnedOption(e.target.dataset.id,e.target.dataset.option,e.target.checked); 
});
document.addEventListener("click", e=>{ 
  if(e.target.classList.contains("favorite-btn")) toggleFavorite(e.target.dataset.id); 
});
