...
function populateFilters(data) {
  ...
  data.forEach(item => {
    ...
    item.characters.forEach(c=>characterSet.add(c));
  });

  seriesSet.forEach(s=>seriesFilter.innerHTML += `<option value="${s}">${s}</option>`);

  const typeOrder = ["立牌","徽章","卡片","杯墊","餐墊","吊飾","收納夾","貼紙","玩偶","模型","海報","掛軸","包、袋","誕生日BOX","其他"];
  typeOrder.forEach(t=>{ if(typeSet.has(t)) typeFilter.innerHTML += `<option value="${t}">${t}</option>`; });

  const charOrder = ["竈門炭治郎","竈門禰豆子","富岡義勇","嘴平伊之助","我妻善逸","栗花落香奈乎","不死川玄彌","胡蝶忍","煉獄杏壽郎","宇髓天元","時透無一郎","甘露寺蜜璃","伊黑小芭內","不死川實彌","悲鳴嶼行冥","產屋敷耀哉","產屋敷輝利哉","鬼舞辻無慘","黑死牟","童磨","猗窩座","半天狗","玉壺","妓夫太郎","墮姬","錆兔","真菰","鱗瀧左近次","神崎葵","煉獄槙壽郎","竈門炭十郎"];
  charOrder.forEach(c=>{ if(characterSet.has(c)){
...
function filterProducts() {
  ...
  if(viewMode==="fav") filtered = filtered.filter(item=>favorites.includes(String(item.id)));
